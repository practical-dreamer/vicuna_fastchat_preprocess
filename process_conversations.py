...
def roles_are_alternating(conversations, human_name, ai_name):
    if not conversations or conversations[0]["from"] != human_name:
        return False

    for i in range(1, len(conversations)):
        if conversations[i]["from"] == conversations[i - 1]["from"] or \
...
def main(args):
    ...
    with open(args.input_json_path, 'r') as f, open(args.output_txt_path, 'w') as out_f:
        ...
        for conversations in tqdm(conversations_generator, total=total_conversations, desc="Processing conversations"):
            ...
            if conversations and roles_are_alternating(conversations, args.human_name, args.ai_name):
